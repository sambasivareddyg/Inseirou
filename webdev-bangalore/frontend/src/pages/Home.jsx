...
const SERVICES = [
  {
    icon: "⚡",
    title: "React & Next.js",
    desc: "Lightning-fast frontends with modern frameworks",
  },
  {
    icon: "☁️",
    title: "Cloud Architecture",
    desc: "Scalable microservices on AWS, GCP & Azure",
  },
  {
    icon: "📱",
    title: "Mobile Apps",
  ...
  },
  {
    ...
    desc: "Real-time dashboards and data pipelines",
  },
];
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-accent-400/10 rounded-full blur-3xl animate-float"
          style={{ animationDelay: "3s" }}
        />

        <div className="relative z-10 text-center max-w-5xl mx-auto">
          <div
            className="inline-flex items-center gap-2 glass px-4 py-2 rounded-full text-xs text-brand-300 font-mono mb-8 animate-fade-in"
            data-delay="0.1s"
          >
          ...
          <h1
            className="text-5xl md:text-7xl lg:text-8xl font-display font-black text-white leading-[0.9] mb-6 opacity-0 animate-fade-up"
            data-delay="0.2s"
          >
            We Build
            <br />
          ...
          <p
            className="text-slate-400 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed opacity-0 animate-fade-up"
            data-delay="0.4s"
          >
          ...
          >
            <Link
              to="/contact"
              className="px-8 py-4 bg-brand-500 hover:bg-brand-600 text-white font-semibold rounded-xl transition-all duration-200 hover:glow hover:scale-105 text-sm"
            >
              Start a Project →
            </Link>
            <Link
              to="/projects"
              className="px-8 py-4 glass text-white font-semibold rounded-xl transition-all duration-200 hover:bg-white/10 text-sm"
            >
              View Our Work
            </Link>
          </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {STATS.map((s) => (
            <div
              key={s.label}
              className="glass rounded-2xl p-6 text-center hover:border-brand-500/40 transition-colors"
            >
              <div className="text-4xl font-display font-black text-gradient mb-1">
                {s.value}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {SERVICES.map((s, i) => (
              ...
              >
                ...
                <h3 className="text-white font-display font-bold text-xl mb-2 group-hover:text-brand-300 transition-colors">
                  {s.title}
                </h3>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {TECH.map((t) => (
              <span
                key={t}
                className="glass px-6 py-3 rounded-full text-sm font-mono text-slate-300 hover:text-brand-300 hover:border-brand-500/40 transition-all cursor-default"
              >
                {t}
              </span>
            ))}
        ...
        <div className="max-w-4xl mx-auto glass rounded-3xl p-12 text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-300 mb-8 max-w-xl mx-auto">
              Let's discuss your project and craft a solution that exceeds
              expectations.
            </p>
            <Link
              to="/contact"
              className="inline-flex px-10 py-4 bg-brand-500 hover:bg-brand-600 text-white font-semibold rounded-xl hover:glow transition-all duration-200 hover:scale-105"
            >
              Get in Touch →
            </Link>
          </div>
